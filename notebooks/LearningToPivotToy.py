...
plotting.plot_toy_variates(X, Y, Z, outfile = "inclusive.pdf")
plotting.plot_toy_variates(X1, Y1, Z1, outfile = "z_1.pdf")
plotting.plot_toy_variates(X0, Y0, Z0, outfile = "z_2.pdf")
plotting.plot_toy_variates(X_1, Y_1, Z_1, outfile = "z_3.pdf")
...
# # input placeholders
# x_in = tf.placeholder(tf.float32, shape=(None, 1), name='X1_X2')
...
# clf_output, vars_D = models.randomized_classifier(x_in, name)
...
# pred = models.classifier_pred(sess, clf_output, x_in, X)
# pred1 = models.classifier_pred(sess, clf_output, x_in, X1)
# pred0 = models.classifier_pred(sess, clf_output, x_in, X0)
# pred_1 = models.classifier_pred(sess, clf_output, x_in, X_1)
...
# # input placeholders
# x_in = tf.placeholder(tf.float32, shape=(None, 1), name='X12')
# y_in = tf.placeholder(tf.float32, shape=(None, ), name='Y')
# z_in = tf.placeholder(tf.float32, shape=(None, ), name='Z')
# inputs = [x_in, y_in, z_in]

# # create the classifier graph, loss, and optimisation
# clf_output, vars_D = models.randomized_classifier(x_in, name+'_clf')
# loss_D = models.classifier_loss(clf_output, y_in)
# opt_D = tf.train.AdamOptimizer(learning_rate=0.01).minimize(loss_D, var_list=vars_D)
...
#opt_R = tf.train.AdamOptimizer(learning_rate=0.0005).minimize(loss_R, var_list=vars_R)
opt_R = tf.train.AdamOptimizer(learning_rate=0.0004).minimize(loss_R, var_list=vars_R)

# create the combined loss function (for the classifier)
loss_DR = loss_D - lam*loss_R
#opt_DR = tf.train.AdamOptimizer(learning_rate=0.0005).minimize(loss_DR, var_list=vars_D)
opt_DR = tf.train.AdamOptimizer(learning_rate=0.0004).minimize(loss_DR, var_list=vars_D)
...
X_single = np.full([10000, 1], -0.2)
pred_single = models.classifier_pred(sess, clf_output, x_in, X_single)
X_single1 = np.full([10000, 1], 0.1666)
pred_single1 = models.classifier_pred(sess, clf_output, x_in, X_single1)
X_single0 = np.full([10000, 1], 0.5)
pred_single0 = models.classifier_pred(sess, clf_output, x_in, X_single0)
X_single_1 = np.full([10000, 1], 0.8333)
pred_single_1 = models.classifier_pred(sess, clf_output, x_in, X_single_1)
test_data['preds_single'] = pred_single, pred_single1, pred_single0, pred_single_1
